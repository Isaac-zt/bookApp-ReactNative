...
// Utility: check connectivity to the Appwrite endpoint (helps diagnose "Network request failed")
export async function checkAppwriteConnection(timeout = 5000) {
    try {
        const controller = new AbortController();
        const id = setTimeout(() => controller.abort(), timeout);

        // A simple GET to the configured endpoint. If the server is reachable
        // this will resolve; otherwise it will throw (network error / timeout).
